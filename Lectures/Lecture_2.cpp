...
int main() {
    ...
    /*
    cout << A[1][2] << endl; // 6
    cout << A << endl; // 0x7ffee2b1cfb0
    cout << A + 1 << endl; // 0x7ffee2b1cfc0
    cout << *(A + 1) << endl; // 0x7ffee2b1cfc0
    cout << &A << endl; // 0x7ffee2b1cfb0
    cout << A[1][0] << endl;
    ...
    */
    ...
    for (int i = 0; i < 5; i++) {
        cout << test[i] << ", "; // test[0] increased by 1
    }
    cout << endl;
    ...
    // It results in memory crash
    int arr[10];
    for (int i = 0; i < 100; i++) { arr[i] = i; }

    // String fundamentals
    // C++ support 2 types of strings: 1. char array ended with null; 2. string class from lib
    char string[11]; // example: a string has 10 characters ended with null
    char web[] = "www.baidu.com";
    char str[6] = {'H', 'e', 'l', 'l', 'o', '\0'};

    // reverse a string
    char *start, *end;
    int len = strlen(web);
    start = web, end = &web[len - 1];
    while (start < end) {
        char t = *start;
        *start = *end;
        *end = t;
